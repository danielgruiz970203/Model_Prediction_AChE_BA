...
def filter_VBA_BO (Data_Frame, Target_Source_Organism):
    ...
    for i in range(len(Data_Frame)):
        if Data_Frame.loc[i,"Target Source Organism According to Curator or DataSource"] == Target_Source_Organism and not Data_Frame.loc[i,"IC50 (nM)"].startswith("<") and not Data_Frame.loc[i,"IC50 (nM)"].startswith(">" or "<") and not Data_Frame.loc[i,"IC50 (nM)"].startswith("nan")  and not Data_Frame.loc[i,"IC50 (nM)"].startswith(" "):

            Molecules_IC50.append ({"BindingDB_Reactant_set_id": Data_Frame.loc[i,"BindingDB Reactant_set_id"],
